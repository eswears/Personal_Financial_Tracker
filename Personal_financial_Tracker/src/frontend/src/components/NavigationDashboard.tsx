...
import { PieChart, Pie, BarChart, Bar, LineChart, Line, AreaChart, Area, Cell, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, RadialBarChart, RadialBar } from 'recharts';
...
// Metric Card Component with Click Navigation
    ...
    >
      ...
      {clickable && (
        <div className="mt-4 text-xs text-blue-600 flex items-center">
          View details <Icon name="arrow" className="w-3 h-3 ml-1" />
        </div>
      )}
    </div>
  );
...
// Spending Distribution with Data
...
// AI Insights Card
const AIInsightsCard: React.FC<{ onViewDetails: () => void }> = ({ onViewDetails }) => {
  const insights = [
    { text: "You could save $240/month by reducing dining out", priority: 'high' },
    { text: "Your emergency fund needs $2,000 more", priority: 'medium' },
    { text: "Consider investing your surplus $500/month", priority: 'low' }
  ];
  ...
  return (
    <div className="bg-gradient-to-br from-purple-50 to-indigo-100 rounded-xl shadow-sm border border-purple-200 p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <Icon name="insights" className="w-6 h-6 text-purple-600" />
      </div>
...
export const NavigationDashboard: React.FC = () => {
  const [activeView, setActiveView] = useState<'overview' | 'transactions' | 'analytics' | 'budget' | 'insights' | 'upload' | 'settings'>('overview');
          ...
          {activeView === 'overview' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <AIInsightsCard onViewDetails={() => setActiveView('insights')} />
              </div>
